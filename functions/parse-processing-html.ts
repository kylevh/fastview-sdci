...
export interface WorkflowStage {
  ...
  isActive: boolean;
  events: PermitEvent[];
  // Only present on the "Reviews" stage
...
function parseEvent(rawText: string): PermitEvent {
  ...
  const date =
    rawEventDate && rawEventDate !== "TBD" ? parseDateStr(rawEventDate) : null;

  return { status, date, dueDate, assignedTo, raw };
}

function parseDateStr(mmddyyyy: string): string | null {
...
function parseEventsFromDetailRow(
  detailRow: ReturnType<typeof parse>,
  isReviewsStage: boolean
): PermitEvent[] {
  const events: PermitEvent[] = [];
  ...
  if (isReviewsStage) {
    ...
    for (const td of eventTds) {
      if (td.querySelector("table table")) continue;
      if (td.text.includes("Marked as")) {
        events.push(parseEvent(td.text));
      }
    }
...
function parseAdditionalInfo(detailRow: ReturnType<typeof parse>): number | null {
...
export function parsePermitStatusHtml(html: string): ParsedPermitStatus {
  const cleaned = html.startsWith('"') ? JSON.parse(html) : html;
  const root = parse(cleaned);

  const workflowStages: WorkflowStage[] = [];
  const disciplineReviews: DisciplineReview[] = [];
  ...
  for (let i = 0; i < allRows.length; i++) {
    ...
    if (!id || !id.match(/^[0-9a-f]{8}-/)) continue;
    ...
    const statusImg = prevRow.querySelector(
      "img[alt='Complete'], img[alt='active']"
    );
    ...
    if (WORKFLOW_STAGE_NAMES.has(name)) {
      const stage: WorkflowStage = { name, isComplete, isActive, events };
...
export function summarizePermit(parsed: ParsedPermitStatus) {
