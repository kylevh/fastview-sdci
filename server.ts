...
import { getUserByToken, loginUser, registerUser } from "./functions/auth-store.js";
import {
  addTrackedPermit,
  listTrackedPermits,
  removeTrackedPermit,
} from "./functions/permit-tracking-store.js";
...
async function fetchPermit(recordNumber: string): Promise<PermitApiResponse> {
  ...
  if (!permitInfo?.capIds?.capID1 || !permitInfo?.capIds?.capID2 || !permitInfo?.capIds?.capID3) {
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    return res.json({ token: user.token, user: { id: user.id, email: user.email, createdAt: user.createdAt } });
  } catch (e: any) {
